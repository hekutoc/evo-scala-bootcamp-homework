...
object ImplicitsHomework {
  ...
  object SuperVipCollections4s {
     ...
     * @tparam V value type
     */
    final class MutableBoundedCache[K: GetSizeScore, V: GetSizeScore](maxSizeScore: Int) {
      ...
      def put(key: K, value: V): Unit = {
        map.put(key, value)
        size += key.sizeScore + value.sizeScore
        while(size > maxSizeScore){
          val (k, v) = map.head
          size -= k.sizeScore + v.sizeScore
          map.remove(k)
        }
      }
    ...
    object instances {
      ...
      implicit def ByteToSizeScore: GetSizeScore[Byte] = (value: Byte) => 1
      implicit def CharToSizeScore: GetSizeScore[Char] = (value: Char) => 2
      implicit def IntToSizeScore: GetSizeScore[Int] = (value: Int) => 4
      implicit def LongToSizeScore: GetSizeScore[Long] = (value: Long) => 8
      implicit def StringToSizeScore: GetSizeScore[String] = (value: String) => 12 + value.length * 'a'.sizeScore
      implicit def ListToSizeScore[T:GetSizeScore]: GetSizeScore[List[T]] = (value: List[T]) =>
        12 + value.iterator.foldLeft(0)((prev, el) => prev + el.sizeScore)

      implicit def VectorToSizeScore[T:GetSizeScore]: GetSizeScore[Vector[T]] = (value: Vector[T]) =>
        12 + value.iterator.foldLeft(0)((prev, el) => prev + el.sizeScore)

      implicit def ArrayToSizeScore[T: GetSizeScore]: GetSizeScore[Array[T]] = (value: Array[T]) =>
        12 + value.iterator.foldLeft(0)((prev, el) => prev + el.sizeScore)

      implicit def MapToSizeScore[T: GetSizeScore, S:GetSizeScore]: GetSizeScore[Map[T, S]] = (value: Map[T, S]) =>
        12+
        mapIterator.iterator1[T,S](value).iterator.foldLeft(0)((prev, el) => prev + el.sizeScore)+
        mapIterator.iterator2[T,S](value).iterator.foldLeft(0)((prev, el) => prev + el.sizeScore)

      implicit def PackedMultiMapToSizeScore[T: GetSizeScore, S:GetSizeScore]: GetSizeScore[PackedMultiMap[T, S]] = (value: PackedMultiMap[T, S]) =>
        12+
          packedMultimapIterator.iterator1[T,S](value).iterator.foldLeft(0)((prev, el) => prev + el.sizeScore)+
          packedMultimapIterator.iterator2[T,S](value).iterator.foldLeft(0)((prev, el) => prev + el.sizeScore)
  ...
  object MyTwitter {
    ...
    implicit def TwitToSizeScore: GetSizeScore[Twit] = (value: Twit) => {
        value.id.sizeScore +
    ...
    implicit def FbiNoteSizeScore: GetSizeScore[FbiNote] = (value: FbiNote) => {
